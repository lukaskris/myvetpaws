...
<?= $this->section('title') ?>Edit Pet | <?= esc($pet['name']) ?><?= $this->endSection() ?>

<?= $this->section('header') ?>Edit Pet Profile<?= $this->endSection() ?>
...
<div class="space-y-6" x-data="petForm">
    ...
    <div class="flex items-center space-x-2 text-xs text-neutral-400">
        ...
        <span>/</span>
        <a href="/pets/show/<?= $pet['id'] ?>" class="hover:text-white transition duration-150"><?= esc($pet['name']) ?></a>
        <span>/</span>
    ...
    <div class="pb-6 border-b border-neutral-900">
        <h2 class="text-xl font-bold text-white tracking-tight">Edit Pet Record</h2>
        <p class="text-xs text-neutral-400 mt-1">Modify patient registration details for <?= esc($pet['name']) ?> (#<?= esc($pet['id']) ?>).</p>
    </div>

    <!-- Form block -->
    <form action="/pets/edit/<?= $pet['id'] ?>" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-2xl" @submit="submitting = true">
        <?= csrf_field() ?>
        ...
        <div class="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl space-y-6 shadow-md">
            ...
            <!-- Pet Photo Upload -->
            <div>
                <label class="block text-xs font-semibold text-neutral-300 mb-2">Pet Photo</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="inp-name" name="name" required value="<?= esc(old('name') ?? $pet['name']) ?>" placeholder="e.g. Fluffy"
                ...
                <div>
                    ...
                    <select id="inp-gender" name="gender"
                        ...
                        <option value="Neutered Male" <?= (old('gender') ?? $pet['gender']) === 'Neutered Male' ? 'selected' : '' ?>>Neutered Male</option>
                        <option value="Spayed Female" <?= (old('gender') ?? $pet['gender']) === 'Spayed Female' ? 'selected' : '' ?>>Spayed Female</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="inp-breed" name="breed" value="<?= esc(old('breed') ?? $pet['breed']) ?>" placeholder="e.g. Persian or Golden Retriever"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="sm:col-span-1">
                    ...
                    <input type="text" id="inp-color" name="color" value="<?= esc(old('color') ?? $pet['color']) ?>" placeholder="e.g. Black & White"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
                <div>
                    <label for="inp-birth" class="block text-xs font-semibold text-neutral-300">Date of Birth</label>
                    <input type="date" id="inp-birth" name="birth_date" value="<?= esc(old('birth_date') ?? $pet['birth_date']) ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
                <div>
                    <label for="inp-vac" class="block text-xs font-semibold text-neutral-300">Last Vaccinated Date</label>
                    <input type="date" id="inp-vac" name="vaccinated_at" value="<?= esc(old('vaccinated_at') ? date('Y-m-d', strtotime($pet['vaccinated_at'])) : '') ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="inp-notes" name="notes" rows="3" placeholder="e.g. Allergic to penicillin. Prefers wet food."
                    class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition"><?= esc(old('notes') ?? $pet['notes']) ?></textarea>
            </div>
        </div>
