...
<?= $this->section('header') ?>Register Customer<?= $this->endSection() ?>
...
<div class="space-y-6" x-data="customerForm">
    ...
    <div class="pb-6 border-b border-neutral-900">
        <h2 class="text-xl font-bold text-white tracking-tight">Register New Customer</h2>
        <p class="text-xs text-neutral-400 mt-1">Create a new customer profile to associate multiple pet records and visit check-ins.</p>
    </div>

    <!-- Form block -->
    <form action="/customers/create" method="POST" enctype="multipart/form-data" class="space-y-6 max-w-2xl" @submit="submitting = true">
        <?= csrf_field() ?>
        ...
        <div class="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl space-y-6 shadow-md">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                ...
                <div class="sm:col-span-3">
                    ...
                    <input type="text" id="inp-name" name="name" required value="<?= old('name') ?>" placeholder="e.g. John Doe"
            ...
            <div>
                <label class="block text-xs font-semibold text-neutral-300 mb-2">Profile Picture (Optional)</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="inp-phone" name="phone" value="<?= old('phone') ?>" placeholder="e.g. +628123456789"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
                <div>
                    <label for="inp-email" class="block text-xs font-semibold text-neutral-300">Email Address</label>
                    <input type="email" id="inp-email" name="email" value="<?= old('email') ?>" placeholder="e.g. customer@example.com"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" id="inp-address" name="address" value="<?= old('address') ?>" placeholder="e.g. Jl. Kemang Raya No. 45"
                    class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-neutral-900">
            <a href="/customers" class="px-4 py-2.5 border border-neutral-800 bg-transparent text-neutral-400 hover:text-white rounded-xl text-xs font-semibold transition">
                Cancel
            </a>
