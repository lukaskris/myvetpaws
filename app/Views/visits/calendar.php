...
<?= $this->section('title') ?>Calendar<?= $this->endSection() ?>

<?= $this->section('header') ?>Calendar<?= $this->endSection() ?>
...
<div class="space-y-6"
     x-data="{
         ...
         getAppointmentLabel(type) {
             const labels = { vet_checkup: 'Vet Checkup', home_visit: 'Home Visit', grooming: 'Grooming' };
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-white tracking-tight">Clinic Calendar</h2>
            <p class="text-sm text-neutral-400 mt-1">Browse scheduled patient appointments across all visit types.</p>
        </div>
        <div>
                    ...
                    <div class="hidden sm:block flex-1 mt-2 space-y-1.5 overflow-y-auto max-h-[70px] custom-scrollbar">
                        <template x-for="event in getEventsForDate(cell.dateStr)" :key="event.id">
                            <div class="px-1.5 py-0.5 text-[9px] font-bold border rounded-lg flex flex-col truncate leading-normal select-none"
                                 :class="[
                                    event.status === 1 ? 'bg-brand-650/15 text-brand-350 border-brand-600/30' : '',
                                    event.status === 2 ? 'bg-amber-500/10 text-amber-400 border-amber-500/25' : '',
                                    event.status === 3 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/25' : '',
                                    event.status === 4 ? 'bg-neutral-850/45 text-neutral-500 border-neutral-800/60' : ''
                                 ]"
                                 :title="`${event.pet_name} - ${event.customer_name}\nType: ${getAppointmentLabel(event.appointment_type)}\nTime: ${event.time}\nComplaint: ${event.complaints}`">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="space-y-1">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span class="px-1.5 py-0.5 text-[8px] font-extrabold uppercase rounded bg-brand-500/10 text-brand-400 border border-brand-500/20" x-text="getAppointmentLabel(event.appointment_type)"></span>
                                    </div>
