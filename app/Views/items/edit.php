...
<div class="max-w-2xl mx-auto space-y-6" x-data="{ submitting: false }">
    ...
    <div class="flex items-center space-x-2 text-xs text-neutral-400">
        ...
        <span>/</span>
        <span class="text-white font-medium">Edit Item</span>
    </div>

    <!-- Header Block -->
    <div class="pb-6 border-b border-neutral-900">
        <h2 class="text-xl font-bold text-white tracking-tight">Edit Inventory Item</h2>
        <p class="text-xs text-neutral-400 mt-1">Modify item details, prices, adjust current stock, or configure active status for <?= esc($item['name']) ?>.</p>
    </div>
    ...
    <div class="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl shadow-lg">
        <form action="/items/edit/<?= $item['id'] ?>" method="POST" class="space-y-6" @submit="submitting = true">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="inp-code" name="code" required value="<?= old('code', $item['code']) ?>"
                ...
                <div>
                    ...
                    <input type="text" id="inp-name" name="name" required value="<?= old('name', $item['name']) ?>"
                ...
                <div>
                    ...
                    <select id="inp-category" name="category" required
                        ...
                        <option value="Medicine" <?= old('category', $item['category']) == 'Medicine' ? 'selected' : '' ?>>Medicine (Obat)</option>
                        <option value="Supplies" <?= old('category', $item['category']) == 'Supplies' ? 'selected' : '' ?>>Supplies (Alat Medis/BHP)</option>
                        <option value="Equipment" <?= old('category', $item['category']) == 'Equipment' ? 'selected' : '' ?>>Equipment (Peralatan)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-1.5 flex rounded-xl bg-neutral-950 border border-neutral-800 focus-within:border-brand-500 focus-within:ring-1 focus-within:ring-brand-500 overflow-hidden">
                        ...
                        <input type="number" id="inp-buy-price" name="buy_price" required step="0.01" min="0" value="<?= old('buy_price', $item['buy_price']) ?>"
                ...
                <div>
                    ...
                    <div class="mt-1.5 flex rounded-xl bg-neutral-950 border border-neutral-800 focus-within:border-brand-500 focus-within:ring-1 focus-within:ring-brand-500 overflow-hidden">
                        ...
                        <input type="number" id="inp-sell-price" name="sell_price" required step="0.01" min="0" value="<?= old('sell_price', $item['sell_price']) ?>"
                            class="flex-1 bg-transparent px-4 py-2.5 text-white placeholder-neutral-600 outline-none text-sm">
                    </div>
                </div>
                ...
                <div>
                    <label for="inp-stock" class="block text-xs font-semibold text-neutral-300">Current Stock Level</label>
                    <input type="number" id="inp-stock" name="stock" required value="<?= old('stock', $item['stock']) ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="inp-min-stock" name="min_stock" required min="0" value="<?= old('min_stock', $item['min_stock']) ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="inp-desc" name="description" rows="3"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition"><?= old('description', $item['description']) ?></textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-neutral-800">
                ...
                <button type="submit" :disabled="submitting"
                    ...
                    <span x-show="!submitting">Update Item</span>
