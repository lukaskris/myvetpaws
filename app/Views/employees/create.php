...
<?= $this->section('title') ?>Add Staff Employee<?= $this->endSection() ?>
...
<div class="max-w-2xl mx-auto space-y-6" x-data="{ submitting: false }">
    ...
    <div class="flex items-center space-x-2 text-xs text-neutral-400">
        ...
        <span>/</span>
        <span class="text-white font-medium">Add Employee</span>
    </div>

    <!-- Header block -->
    <div class="pb-6 border-b border-neutral-900">
        <h2 class="text-xl font-bold text-white tracking-tight">Add Staff Employee</h2>
        <p class="text-xs text-neutral-400 mt-1">Provision a new user account with designated roles to allow operations access.</p>
    </div>
    ...
    <div class="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl shadow-lg">
        <form action="/employees/create" method="POST" class="space-y-6" @submit="submitting = true">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" id="inp-email" name="email" required value="<?= old('email') ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition"
                ...
                <div>
                    ...
                    <input type="tel" id="inp-phone" name="phone" value="<?= old('phone') ?>"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition"
                ...
                <div>
                    ...
                    <select id="inp-role" name="role" required
                        ...
                        <option value="doctor" <?= old('role') == 'doctor' ? 'selected' : '' ?>>Doctor (Clinical Operations)</option>
                        <option value="receptionist" <?= old('role') == 'receptionist' ? 'selected' : '' ?>>Receptionist (Front Desk)</option>
                        <option value="finance" <?= old('role') == 'finance' ? 'selected' : '' ?>>Finance (Billing & Reports)</option>
                    </select>
                </div>

                <!-- Password -->
                <div>
                    <label for="inp-password" class="block text-xs font-semibold text-neutral-300">Account Password</label>
                    <div class="mt-1.5 relative" x-data="{ show: false }">
                        <input :type="show ? 'text' : 'password'" id="inp-password" name="password" required
