...
<div class="max-w-2xl mx-auto space-y-6" x-data="{ submitting: false }">
    ...
    <div class="flex items-center space-x-2 text-xs text-neutral-400">
        ...
        <span>/</span>
        <a href="/services" class="hover:text-white transition duration-150">Services</a>
        <span>/</span>
        <span class="text-white font-medium">Edit Service</span>
    </div>

    <!-- Header Block -->
    <div class="pb-6 border-b border-neutral-900">
        <h2 class="text-xl font-bold text-white tracking-tight">Edit Clinic Service</h2>
        <p class="text-xs text-neutral-400 mt-1">Modify pricing template and status configuration for <?= esc($service['name']) ?>.</p>
    </div>
    ...
    <div class="bg-neutral-900 border border-neutral-800 p-6 rounded-3xl shadow-lg">
        <form action="/services/edit/<?= $service['id'] ?>" method="POST" class="space-y-6" @submit="submitting = true">
            <?= csrf_field() ?>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <!-- Service Code -->
                <div>
                    <label for="inp-code" class="block text-xs font-semibold text-neutral-300">Service Code</label>
                    <input type="text" id="inp-code" name="code" required value="<?= old('code', $service['code']) ?>"
                ...
                <!-- Service Name -->
                <div>
                    <label for="inp-name" class="block text-xs font-semibold text-neutral-300">Service Name</label>
                    <input type="text" id="inp-name" name="name" required value="<?= old('name', $service['name']) ?>"
                ...
                <div>
                    ...
                    <select id="inp-category" name="category" required
                        ...
                        <option value="Laboratory Test" <?= old('category', $service['category']) == 'Laboratory Test' ? 'selected' : '' ?>>Laboratory Test</option>
                        <option value="Medicine" <?= old('category', $service['category']) == 'Medicine' ? 'selected' : '' ?>>Medicine</option>
                        <option value="Supplies" <?= old('category', $service['category']) == 'Supplies' ? 'selected' : '' ?>>Supplies</option>
                    </select>
                </div>

                <!-- Price -->
                <div>
                    <label for="inp-price" class="block text-xs font-semibold text-neutral-300">Base Price (IDR)</label>
                    <div class="mt-1.5 flex rounded-xl bg-neutral-950 border border-neutral-800 focus-within:border-brand-500 focus-within:ring-1 focus-within:ring-brand-500 overflow-hidden">
                        ...
                        <input type="number" id="inp-price" name="price" required step="0.01" min="0" value="<?= old('price', $service['price']) ?>"
                ...
                <!-- Status -->
                <div>
                    <label for="inp-status" class="block text-xs font-semibold text-neutral-300">Status</label>
                    <select id="inp-status" name="status" required
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="inp-desc" name="description" rows="3"
                        class="mt-1.5 block w-full rounded-xl bg-neutral-950 border border-neutral-800 px-4 py-2.5 text-white placeholder-neutral-500 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm transition"><?= old('description', $service['description']) ?></textarea>
                </div>
            </div>

            <!-- Buttons -->
            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-neutral-800">
                <a href="/services" class="px-4 py-2.5 border border-neutral-800 bg-transparent text-neutral-400 hover:text-white rounded-xl text-xs font-semibold transition">
                    Cancel
                </a>
                <button type="submit" :disabled="submitting"
                    ...
                    <span x-show="!submitting">Update Service</span>
